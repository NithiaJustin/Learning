...
weekend_days = filter(is_weekend, weatherdata)
warmest_weekend = max(weekend_days, key=lambda d :d['tmax'])
#print(date.fromisoformat(warmest_weekend['date']).strftime('%a, %d %b %Y '))
...
for _ in range(7):
    ...
    avg_temp += (wd['tmax']+ wd['tmin'])/2
avg_temp = avg_temp/7

print(f'The average temperature for the next seven days was {avg_temp}')